...
void test_SSD(int fd, unsigned int block_size, double *read_result, double *write_result)
{
    ...
    struct stat stats;
    fstat(fd, &stats);
    int align = stats.st_blksize - 1;
    int *buf = malloc(buf_size * sizeof(int) + align);
    int *block = (int *)(((__UINTPTR_TYPE__)buf + align) & ~((__UINTPTR_TYPE__)align));
    struct timespec start, finish;
    double sec = 0, nsec = 0;
    for (int i = 0; i < buf_size; i++)
    {
    ...
    clock_gettime(CLOCK_REALTIME, &start);
    ssize_t wrote = write(fd, block, block_size);
    clock_gettime(CLOCK_REALTIME, &finish);
    if (wrote == -1)
    {
        perror("write");
    ...
    *write_result = sec + nsec / 1E9;
    ...
    lseek(fd, 0, SEEK_SET);
    clock_gettime(CLOCK_REALTIME, &start);
    ssize_t readed = read(fd, block, block_size);
    clock_gettime(CLOCK_REALTIME, &finish);
    if (readed == -1)
    {
        perror("read");
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < launch_count; i++)
    {
        ...
        {
            int fd = open("test_file.dat", O_RDWR | O_CREAT | __O_DIRECT, S_IRWXU);
            if (fd == -1)
            {
                perror("open");
                exit(EXIT_FAILURE);
        ...
        {
            int fd = open("/media/sofya/TRANSCEND/test_file.dat", O_RDWR | O_CREAT | __O_DIRECT, S_IRWXU);
            if (fd == -1)
            {
                perror("open");
                exit(EXIT_FAILURE);
