...
void test_RAM(unsigned int block_size, double *read_result, double *write_result)
{
    ...
    *write_result = sec + nsec / 1E9;
    ...
    // sprintf(log_message,"Write: %ld - lsec, %f - dsec, %ld - lnsec, %f - dnsec, %f - results\n",finish.tv_sec - start.tv_sec,sec,finish.tv_nsec-start.tv_nsec,nsec,*write_result);
    ...
    *read_result = sec + nsec / 1E9;
    // sprintf(log_message,"Read: %ld - lsec, %f - dsec, %ld - lnsec, %f - dnsec, %f - results\n",finish.tv_sec - start.tv_sec,sec,finish.tv_nsec-start.tv_nsec,nsec,*read_result);
    // write(log,log_message,strlen(log_message));
    free(buf);
    free(block);
    //close(log);
